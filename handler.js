...
if (this.user && this.user.jid) {
    ...
    console.error("🌿 this.user.jid tidak didefinisikan.")
}
        //---- AA    
       ...
       if (settings.sologp && !m.chat.endsWith('g.us') && !/jadibot|bebot|getcode|serbot|bots|stop|support|donate|off|on|s|tiktok|code|newcode|join/gim.test(m.text)) return
        ...
        for (let name in global.plugins) {
            ...
            if ((usedPrefix = (match[0] || '')[0])) {
                ...
                if (xp > 200)
                    m.reply('suara -_-') // Hehehe
                else
                    m.exp += xp
                if (!isPrems && plugin.limit && global.db.data.users[m.sender].limit < plugin.limit * 1) {
                    this.reply(m.chat, `✳️ limit anda habis\nGunakan perintah berikut untuk membeli lebih banyak berlian\n\n*${usedPrefix}buy*`, m)
                    continue // Limit habis
                }
                if (plugin.level > _user.level) {
                    this.reply(m.chat, `✳️ level yang di perlukan ${plugin.level} untuk menggunakan perintah ini. \nlevel kamu ${_user.level}`, m)
...
global.dfail = (type, m, conn) => {
    let msg = {
      rowner:  `┌─⭓「 *ROWNER ONLY* 」
│ *• Msg :* this feature only for Rowner!
└───────────────⭓`,
      owner:  `┌─⭓「 *OWNER ONLY* 」
...
└───────────────⭓`,
      restrict: `┌─⭓「 *OWNER COMAMAND* 」
│ *• Msg :* Fitur Ini Di matikan !
└───────────────⭓`,
...
└───────────────⭓`,
    ...
    /*if (msg) return conn.sendButton(m.chat, msg, null, [['🔖 OK', 'khajs'], ['⦙☰ MENU', '/menu'] ], rpl)
}*/
