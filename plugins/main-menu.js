...
let handler = async (m, { conn, usedPrefix: _p, __dirname }) => {
  try {
    ...
    let after = conn.menu.after || (conn.user.jid == conn.user.jid ? '' : `⭐ Powered by Jarot https://wa.me/${conn.user.jid.split`@`[0]}`) + defaultMenu.after
    let _text = [
      before,
    ...
    /*conn.sendButton(m.chat, text.trim(), `▢ DyLux  ┃ ᴮᴼᵀ`, pp, [
      ['ꨄ︎ Apoyar', `${_p}donate`],
      ['⏍ Info', `${_p}botinfo`],
      ['⌬ Grupos', `${_p}gpdylux`]
    ], m, rpl)*/
    ...
    conn.reply(m.chat, '❎ MENU SEDANG EROR', fwc)
    throw e
  }
}
handler.help = ['help']
