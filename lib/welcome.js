...
const OscarBot = join(__dirname, '..', 'OscarBot')
const _svg = readFileSync(join(OscarBot, 'welcome.svg'), 'utf-8')
const barcode = data => {
...
const genSVG = async ({
    wid = '',
    pp = join(OscarBot, 'avatar_contact.png'),
    title = '',
    name = '',
    text = '',
    background = ''
...
const render = async ({
    wid = '',
    pp = toBase64(readFileSync(join(OscarBot, 'avatar_contact.png')), 'image/png'),
    name = '',
    title = '',
    text = '',
    background = toBase64(readFileSync(join(OscarBot, 'Aesthetic', 'Aesthetic_000.jpeg')), 'image/jpeg'),
} = {}, format = 'png') => {
